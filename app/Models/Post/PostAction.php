...
{
    public static function post($title, $slug, $content, $category, $status, $type, $image, $tags, $date)
    {
    ...
    public static function delete_comment($id)
    {
        ...
        $article_comments = Article::where('id', $comment->article_id)->first()->comment_count - 1;
    ...
    public static function category($name)
    {
        if(Category::where('name', $name)->exists())
        {
            ...
            $category->name = $name;
            $category->slug = PostAction::slug($name);
    ...
    public static function counter($slug)
    {
        Article::where('slug', $slug)->increment('counter');
    }

    public static function reply($articleId, $commentId, $replyContent)
    {
        $reply = new Comment();
        ...
        $reply->created_by_ip = Crypt::encrypt(LAS::ip());
        $reply->created_by_os = Crypt::encrypt(LAS::os());
        $reply->created_by_browser = Crypt::encrypt(LAS::browser());
        $reply->save();
